...
import { type Task, type User, type HistoryEntry, HistoryActionType, TaskStatus } from "@/types/task"

interface ExplanatoryNoteDialogProps {
  task: Task
  open: boolean
  onOpenChange: (open: boolean) => void
  onUpdateTask: (task: Task, historyAction?: Omit<HistoryEntry, "id" | "timestamp">) => void
  currentUser: User
}
...
export default function ExplanatoryNoteDialog({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const historyAction = {
        actionType: HistoryActionType.EXPLANATORY_NOTE_SENT,
        userId: currentUser.id,
        description: `Отправлена объяснительная записка по просрочке: "${explanation.trim()}"`,
        additionalData: {
          explanation: explanation.trim(),
          overdueDate: task.dueDate,
          daysPastDue: Math.ceil((new Date().getTime() - task.dueDate.getTime()) / (1000 * 60 * 60 * 24))
        }
      }
  ...
  const daysPastDue = Math.ceil((new Date().getTime() - task.dueDate.getTime()) / (1000 * 60 * 60 * 24))
              ...
              <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                ...
                <div className="text-sm text-red-700 space-y-1">
                  <div>
                    ...
                    {format(task.dueDate, "d MMMM yyyy, HH:mm", { locale: ru })}
                  </div>
                  <div>
